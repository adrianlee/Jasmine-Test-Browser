...
function prepare(job, callback) {
    var PACKAGER_URL = "http://mac-ci:9000/job/" + job + "/ws/target/zip/*zip*/zip.zip",
        FUNCTIONAL_TEST_URL = "http://mac-ci:9000/job/" + job + "/Framework/ext/test.functional/zip/*zip*/test.functional.zip";
    ...
    downloadDependency(PACKAGER_URL, function (err) {
        if (err) {
            callback(err);
        } else {
            unzipDependency(function (err) {
                if (err) { 
                    callback(err);
                } else {
                    fs.unlinkSync(_workspace + '/dependency.zip');
                }
            });
        }
    });
    ...
    downloadDependency(FUNCTIONAL_TEST_URL, function (err) {
        if (err) {
            callback(err);
        } else {
            unzipDependency(function (err) {
...
function downloadDependency(source, callback) {
    var _url = url.parse(source),
        destination = _workspace + "/dependency.zip",
        req;
...
function unzipDependency(callback) {
    var data, 
        filesObj, 
        p, 
        parent, 
    ...
    if (!path.existsSync(_workspace + "/dependency.zip")) {
        // throw (new Error(".zip is missing ..."));
        callback('.zip is missing ...');
    }

    data = fs.readFileSync(_workspace + "/dependency.zip");
