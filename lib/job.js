...
function prepare(job, callback) {
    ...
    downloadDependency(EXECUTABLES_URL, EXECUTABLES_FILENAME, function (err) {
        if (err) {
            callback(err);
        } else {
            unzipDependency(EXECUTABLES_FILENAME, function (err) {
                if (err) { 
                    callback(err);
                } else {
                    fs.unlinkSync(_workspace + EXECUTABLES_FILENAME);           
                    // deletes existing folder in dependencies
...
function package(callback) {
    ...
    // BBWP Package
    var cmd = "cd " + _widget + " && " + "zip widget.zip config.xml" + " && " +
            _workspace + "/zip/bbwp " + _widget + "/widget.zip";
    console.log(cmd);
